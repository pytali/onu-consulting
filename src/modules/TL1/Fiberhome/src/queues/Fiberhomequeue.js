...
const fiberhomequeue = new Queue('fh_telnet', {
    redis: {
...
for (let i = 0; i < config.worker.concurrency; i++) {
    const worker = new Worker(new URL('../workers/FHtelnetWorker.js', import.meta.url), {
...
// console.log(fiberhomequeue)


fiberhomequeue.process(config.worker.concurrency, async (job, done) => {
